...
var CelestialBody = function (obj) {
    ...
    // Orbit parameters
    // Period (sidereal), semi-major axis, eccentricity, inclination,
    // longitude of ascending node, mean anomaly (offset at reference epoch assuming circular orbit)
    ...
    // Rotation parameters
    // Period (sidereal), axial tilt (obliquity), meridian angle
    // (angle between the meridian plane of the rotation axis and the xOy plane), offset at epoch.
    // Note: we use the xOz plane as the ecliptic.
    this.rotation = {
        period: 1., inclination: 1.,
        meridianAngle: 0., offset: 0.
    };
    // Parameters for showing glow when viewed from far
    // albedo is reflectivity
    // A rough estimate for how bright this point should be rendered (for visualization only)
    // x > R/k:   (2 - <c, p>/(|c|*|p|)) * R^2 * a * log(k*x0/R) / log(k*x/R)
    // else:    0
    // where a is albedo, <,> denotes inner product, |.| is Euclidean norm,
    // c is camera position, p is body position
    // R is body radius, x is distance |c - p|, and k is a coefficient
    this.albedo = 1.;
