...
tween = new TWEEN.Tween(posSrc)
    ...
    .onComplete(function () {
        ...
        var eligible = ["Sun","Mercury","Venus","Earth","Mars","Jupiter","Saturn","Uranus","Neptune","Pluto"];
        if (eligible.indexOf(curBody) !== -1) {
            try { if (promptSound && typeof promptSound.play === 'function') { promptSound.play(); } } catch (e) {}
            var el = document.getElementById('arrival');
            if (el) {
                el.textContent = curBody;
                el.classList.add('arrival-show');
                window.clearTimeout(el._hideTimer);
                el._hideTimer = window.setTimeout(function(){ el.classList.remove('arrival-show'); }, 2500);
            }
        }
        globalTimeFlag = true;
    });
